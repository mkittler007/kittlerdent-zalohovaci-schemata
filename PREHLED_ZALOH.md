...
6. **Úklid VM balíků na `.120` (5.9.2026) — NEbyla to chyba retence.** Ke 5.9. ležely v `VM_packages` **4 ram balíky** — což byl korektní stav STARÉHO schématu (`obden ram, ret4`); nové schéma (`cold 3 + ram 1`) se aktivovalo teprve 5.9. v 15:12 (plisty `.bak.2026-09-05_schema`) a ram se prořezává až ve čtvrtek. Logika prořezávání ve `prenos_na_synology.sh` (`ls -1dt … | tail -n +KEEP+1 | rm`) je **správná** pro cold i ram — ve skriptu není co opravovat. Ručně srovnáno na cílový stav (smazány 3 staré ram ~1,25 TB) + odstraněn osamocený `macOS_2026-08-24_0732.macvm` (1,7 TB) z **kořene** `/VM macOS M4/` (ruční pozůstatek mimo `VM_packages/`, který skript needituje) + `.DS_Store` → **uvolněno ~2,9 TB** (`.120` z 57 % na 29 %). Pozn.: skript prořezává jen `VM_packages/`, ne rodičovskou složku — ruční pozůstatky v kořeni hlídat okem.
