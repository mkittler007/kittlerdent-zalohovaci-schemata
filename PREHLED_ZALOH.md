...
| 17 | **Time Machine hostu (Mac mini Pro)** | host .24 (VM vyloučená) | **týdně na 2 cíle:** WD `My Book` (disk7s2) + Synology `VM macOS M4` (smb, síťový, do podsložky `TM z MacMiniPro`) | **neděle** WD 03:00 + Synology 14:00 (LaunchDaemony `tm-wd-weekly`/`tm-syno-weekly`; hodinové auto VYPNUTO) | WD **kvóta 3 TB**; NAS **kvóta 1,5 TB** (nastavit po 1. záloze), pak FIFO | nativní TM restore; rotaci WD hlídá `com.kittler.tm-wd-rotation` |
...
| **Synology .120** `/volume1/` | `VM macOS M4/VM_packages` (cold 3 + ram 1), `VM macOS M4/VM_cold_offsite` (zdroj C2 #18), `VM macOS M4/TM z MacMiniPro` (host TM #17), `VM Imac_zelený` (zdroj C2 #19), `Claude_Project` (mirror), `HDD IMac ordinace/Parallels_VM_zaloha`, `npgroup_backup`, `Mac_mini_Pro_Logy`, zdrojové share pro HyperBackup | hlavní on-site NAS, RAID5/Btrfs |
