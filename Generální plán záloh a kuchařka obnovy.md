...
> **⚠️ ZMĚNA PROBÍHÁ (22.9.2026) — nové VM cold → C2 sady.** Martin založil dvě nové HyperBackup sady místo/vedle starých #64/#65: **`VM M4_Pro_Synology` (repo 66)** a **`VM Zelený_Imac_Synology` (repo 67)**, cíl = 1×/týden cold VM do C2. Stará „VM Maco M4 sada ze synology" **SMAZÁNA** (dělala problémy). Cílový stav: M4 zdroj jen `VM_cold_offsite`, rozvrh **týdně Ne 03:00**; Zelený jen aktuální VM (potřeba `offsite_current`), rozvrh **týdně St 03:30**. **Teď jsou obě omylem DENNÍ** (M4 23:10, Zelený 21:20) a M4 zálohuje celý share → **nutno doladit v DSM UI** (`synoschedtask` neumí `--set`). **Staré #64/#65 SMAZÁNY** (22.9. v UI, ověřeno bez osiřelých záznamů) → 66/67 jsou jediné VM offsite sady. **⚠️ M4 dočasně bez platné C2 kopie** (#64 smazána, #66 ještě neproběhla; lokálně M4 chráněná dál) — zavřít opravou zdroje #66 na cold + prvním během. Detail → `KUCHARKA_ZALOH_NAS120.md` §4c. (Pravidlo: změna zálohy = vždy update tohoto plánu, memory `feedback_zalohy_aktualizuj_schema`.)
...
|---|---|---|---|---|---|
| **VM macOS (.82)** | — | cold 3 | cold 4 + ram 1 | cold 3 | ⚠️ cold (#66, v přechodu — #64 smazána, #66 zatím neproběhla) |
| **Host .24** | — | TM | (VM balíky) | TM + Claude/IS/IMS | ✅ Claude/IS/IMS |
| **iMac zelený .170** | cold 1 | — | — | daily 10 | ⚠️ (#67, v přechodu — #65 smazána, #67 zatím neproběhla) |
| **iMac žlutý** | — | TM | — | — | ❌ |
